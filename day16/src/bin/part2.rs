...
fn main() -> io::Result<()> {
    ...
    let flat_scores = node_scores.iter().flat_map(|(k, v)| v.iter()).fold(
        HashMap::new(),
        |mut acc, (k, (v1, v2))| {
            let mut score = *v1;

            if let Some(val) = acc.get(k) {
                score = score.max(*val);
            }

            acc.insert(k.to_owned(), score);
            acc
        },
    );

    let mut max_score = 0;
    for (k1, v1) in &flat_scores {
        for (k2, v2) in &flat_scores {
            if k1.is_disjoint(&k2) {
                max_score = max_score.max(v1 + v2)
            }
        }
    }
