...
fn solve(
    ...
    for s in strats.iter() {
        ...
        if s.or < MAX_NUM && or_or_cost <= s.or {
            next_strats.push(Strategy {
                or: or - or_or_cost,
                cl,
                ob,
                ge,
        ...
        if s.cl < MAX_NUM && cl_or_cost <= s.or {
            next_strats.push(Strategy {
                or: or - cl_or_cost,
                cl,
                ob,
        ...
        if s.ob < MAX_NUM && (ob_or_cost <= s.or && ob_cl_cost <= s.cl) {
            next_strats.push(Strategy {
                or: or - ob_or_cost,
                cl: cl - ob_cl_cost,
                ob,
        ...
        if ge_or_cost <= s.or && ge_ob_cost <= s.ob {
            next_strats.push(Strategy {
                or: or - ge_or_cost,
                cl,
                ob: ob - ge_ob_cost,
...
fn dedup_strats(strats: &mut Vec<Strategy>) {
    let set: HashSet<_> = strats.drain(..).collect();
    strats.extend(set.into_iter());
}

fn cull_strats(strats: &mut Vec<Strategy>) -> Vec<Strategy> {
    dedup_strats(strats);
    let mut removed = Vec::new();
    {
        for (i, s1) in strats.iter().enumerate() {
            for (j, s2) in strats.iter().enumerate() {
                ...
                {
                    removed.push(s2.clone());
                }
            }
        }
    }

    let mut newstrat = Vec::new();
    for s in strats.iter() {
        if removed.contains(&s) {
        } else {
            newstrat.push(s.clone());
        }
    }
    newstrat
}
